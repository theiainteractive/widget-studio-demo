...
#include "Widgets/WSButtonGroup.h"

#define LOCTEXT_NAMESPACE "WidgetStudio"

#if WITH_EDITOR
const FText UWidgetStudioButtonGroup::GetPaletteCategory()
{
...
void UWidgetStudioButtonGroup::UpdateButtonCheckedStates()
{
	...
	for (int i = 0; i < ManagedButtons.Num(); i++)
	{
		...
		{
			// Checked fail safe. This is in case that the button is support to be checked, but for some reason wasn't.
...
void UWidgetStudioButtonGroup::SetCurrentIndex(const int32 Index)
{
